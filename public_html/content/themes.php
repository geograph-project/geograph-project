...
if (!$smarty->is_cached($template, $cacheid))
{
	...
	if ($source == 'words') {
		$listall = $db->getAll("select title from content WHERE `source` IN ('article','gallery','geotrips') AND type = 'info' $where");
		...
		arsort($a);
		$a = array_slice($a,0,200);
		ksort($a);

		$smarty->assign('words', $a);
		...
		if ($source == 'article') {
			$list = $db->getAll($sql = "
	                select user_id,CONCAT('/article/',url) as url,title,extract,update_time as updated,create_time as created,realname,category_name as label
			from article
				inner join article_cat using (article_cat_id)
				left join user using (user_id)
			where `type` = 'info' and article_cat_id not in (5,18) and approved > 0 and licence != 'none'
			order by sort_order,category_name,title");
		} else {
			$list = $db->getAll($sql = "
			...
			from content_group
				...
				left join user using (user_id)
			where content_group.`source` like '$source' and `type` = 'info' AND label != 'Other Topics' $where
			group by content_id,label
			order by content_group.label,content_group.score desc,content_group.sort_order	");
		}

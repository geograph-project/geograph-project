...
{if $userimages}
	...
	<ul>
		...
		<li><b>Karten</b>: {if $profile->stats.images gt 10}<a href="/profile/{$profile->user_id}/map" rel="nofollow">Persönliche Geograph-Karte</a> (<a href="/profile/{$profile->user_id}/mmap" rel="nofollow">zonenlos</a>), {/if} <a href="https://kurpfalzradler.de/geograph/GeographPhotos_userid.html?userid={$profile->user_id}" rel="nofollow">Karte von Kurpfalzradler</a> oder aktuelle Fotos auf <a href="http://maps.google.de/maps?q=http://{$http_host}/profile/{$profile->user_id}/feed/recent.kml&amp;ie=UTF8&amp;om=1">Google Maps</a></li>
		...
		<li><b>Kalender</b>: <a href="/explore/calendar.php?u={$profile->user_id}" rel="nofollow">Persönlicher Kalender</a></li>
		{/if}

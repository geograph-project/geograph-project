...
{if $marker}
	...
	{else}
		<div style="position:absolute;top:{$marker->top-8}px;left:{$marker->left-8}px;">{if $map_token}<a href="/mapbrowse2.php?t={$map_token}{if $image}&amp;gridref_from={$image->grid_reference}{/if}">{/if}<img src="//{$static_host}/img/crosshairs.gif" alt="+" width="16" height="16"/>{if $map_token}</a>{/if}</div>
	{/if}
{/if}
</div>
</div>

...
{/if}
 	...
 	{elseif $user->user_id ne $profile->user_id}
		...
		{/if}
		<li><a title="Contact {$profile->realname|escape:'html'}" href="https://{$http_host}/usermsg.php?to={$profile->user_id}">Send message to {$profile->realname|escape:'html'}</a></li>
	{else}
...
{if $user->user_id eq $profile->user_id}
	<p><a href="https://{$http_host}/profile.php?edit=1">Edit your profile</a> if there's anything you'd like to change.</p> 	
{else}
	<br/><br/>
{/if}
...
{if $userimages}
	...
	<ul>
		...
		<li><b>Maps</b>: {if $profile->stats.images gt 10}<a href="/profile/{$profile->user_id}/map" rel="nofollow">Personalised Geograph Map</a> (<a href="/profile/{$profile->user_id}/mmap" rel="nofollow">no zones</a>), {/if} <a href="https://kurpfalzradler.de/geograph/GeographPhotos_userid.html?userid={$profile->user_id}" rel="nofollow">map by Kurpfalzradler</a> or Recent Photos on <a href="http://maps.google.de/maps?q=http://{$http_host}/profile/{$profile->user_id}/feed/recent.kml&amp;ie=UTF8&amp;om=1">Google Maps</a></li>
		...
		<li><b>Calendar</b>: <a href="/explore/calendar.php?u={$profile->user_id}" rel="nofollow">View personal calendar</a></li>
		{/if}

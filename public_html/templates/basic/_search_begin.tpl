...
{if $engine->error}
	<div class="interestBox" style="padding:2px;border:1px solid red; text-align:center; background-color:pink;color:black;">
		...
		{else}this is most likely an invalid combination of search terms,<br/>
...
{if $engine->resultCount}
	<br/><span class="pageLinks">( Page {$engine->pagesString()})</span> {if $engine->criteria->searchclass != 'Special'}[<a href="/search.php?i={$i}&amp;form=advanced">refine search</a>]{/if}
{/if}

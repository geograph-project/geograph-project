...
<td>{$image->title}</td>
<td sortvalue="{$image->submitted}" style="white-space:nowrap">{$image->submitted|date_format:"%a, %e %b %Y"}</td>
<td>{if $image->moderation_status eq "accepted"}supplemental{else}{$image->moderation_status}{/if}</td>

...
{if $tags}
	...
	TAGS: {foreach from=$tags item=item}
		{if $item.tag eq $thetag}
			<span class="nowrap">&nbsp;<b>{$item.tag|escape:'html'|replace:' ':'&middot;'}</b> [<a href="{$script_name}{if isset($item.prefix)}?prefix={$item.prefix|escape:'url'}{/if}">remove filter</a>] &nbsp;</span>
		{else}
			&nbsp;<a title="{$item.images} images" {if $item.images > 10} style="font-weight:bold"{/if} href="{$script_name}?{if isset($item.prefix)}prefix={$item.prefix|escape:'urlplus'}&amp;{/if}tag={$item.tag|escape:'urlplus'}">{$item.tag|escape:'html'|replace:' ':'&middot;'}</a> &nbsp;
		{/if}
	{/foreach}
	</div>
{/if}
{if $thetag}
	...
	<div class="interestBox">
		...
		{if $theprefix}
			Prefix: <b>{$theprefix|escape:'html'}</b> &nbsp;&nbsp;
		{/if}
	...
	{if $others}
		<div style="font-size:0.8em;text-align:center">Other tags: {foreach from=$others item=item}
			<span class="tag">{if $item.prefix}{$item.prefix|escape:'html'}:{/if}<a href="{$script_name}?{if $item.prefix}prefix={$item.prefix|escape:'urlplus'}&amp;{/if}tag={$item.tag|escape:'urlplus'}&amp;exact=1" class="taglink">{$item.tag|escape:'html'}</a></span>
			&nbsp; &nbsp;
		{/foreach}
		</div>
	{/if}
...
{if $results}
	{if $search_link}
		<p><a href="{$search_link}">View as Search Results</a> create a search result based on the images shown below</p>
	{/if}

	{if !$example && !$private}
		<p><b>Showing {if $images > 50}{if $gridref}nearest{else}latest{/if} 50 of {$images|thousends}{/if} images tagged with <span class="tag">{if $theprefix}{$theprefix|escape:'html'}:{/if}<a class="taglink">{$thetag|escape:'html'}</a></span> tag{if $gridref} near <a href="/gridref/{$gridref}">{$gridref}</a>{/if}{if $exclude} but excluding images with <span class="tag"><a class="taglink">{$exclude|escape:'html'}</a></span> tag{/if}.</b></p>
	...
	{if !$example && !$private}
		<div style="text-align:right">
			...
			<a href="/search.php?text=tags:{$thetag|escape:'url'}">View images matching '{$thetag|escape:'html'}' anywhere <i>in</i> the tags</a>
		</div>
	{/if}
...
{elseif $thetag}
	...
	&middot; <a href="/search.php?text=tags:{$thetag|escape:'url'}">Search for images matching '{$thetag|escape:'html'}' anywhere <i>in</i> the tags</a>
{/if}

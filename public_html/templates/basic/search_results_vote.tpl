...
{if $engine->resultCount}
	...
	{searchbreak image=$image}
	 <div style="border-top: 1px solid lightgrey; padding-top:1px;max-width:940px">
	  <div style="float:left; position:relative; width:130px; text-align:center">
		<a title="{$image->title|escape:'html'} - click to view full size image" href="/photo/{$image->gridimage_id}">{$image->getThumbnail(120,120)}</a>
	  </div>
	  <div style="float:right; position:relative" id="votediv{$image->gridimage_id}"><b>Rate this image</b>: {votestars id=$image->gridimage_id type="i`$i`"}</div>

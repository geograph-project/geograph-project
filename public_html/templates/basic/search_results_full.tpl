	...
	{searchbreak image=$image}
	 <div style="border-top: 1px solid lightgrey; padding-top:1px;">
	  ...
	  <div style="float:left; position:relative">
		...
		{if $image->moderation_status == 'geograph'}geograph {if $image->ftf}(first){/if}{else}{if $image->moderation_status == 'pending'}pending{/if}{/if} for square <a title="view page for {$image->grid_reference}" href="/gridref/{$image->grid_reference}">{$image->grid_reference}</a>

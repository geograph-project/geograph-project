...
{foreach from=$list item=item}
	...
	<small><small style="color:gray">{if $item.type != 'article' && $item.type != 'help'}started{/if} by <a href="/profile/{$item.user_id}" title="View Geograph Profile for {$item.realname}"  style="color:#6699CC" target="_top">{$item.realname}</a>.</small></small>
	{if $item.extract}

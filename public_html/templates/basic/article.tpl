...
{foreach from=$list item=item}
	...
	<li><b>{if !$item.approved || $item.licence == 'none'}<s>{/if}<a title="View {$item.title}" href="/article/{$item.url}">{$item.title}</a></b>{if !$item.approved || $item.licence == 'none'}</s> (Not publicly visible){/if}

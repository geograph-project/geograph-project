...
{elseif $licence == 'cc-by-sa/2.0'}
	<!-- Creative Commons Licence -->
		...
		alt="Creative Commons Licence [Some Rights Reserved]" src="http://{$static_host}/img/somerights20.gif" /></a> &nbsp; Text &copy; Copyright {$publish_date|date_format:" %B %Y"}, <a href="/profile/{$user_id}" title="View Geograph Profile for {$realname}">{$realname}</a>; 
...
{else}
	...
	<img alt="Creative Commons License" style="border-width:0" src="http://{$static_host}/img/pd-88x31.png" /></a> {/if} Text by <a href="/profile/{$user_id}" title="View Geograph Profile for {$realname}">{$realname}</a>, {$publish_date|date_format:" %B %Y"}

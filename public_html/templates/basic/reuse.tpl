	...
	<tr>
		...
		<td>
			<a href="{$tile_host}/stamp.php?id={$image->gridimage_id}{if $image->cached_size.0 > 500}&title=on{/if}&gravity=SouthEast&hash={$image->_getAntiLeechHash()}&download=1"><img 
                            src="{$tile_host}/stamp.php?id={$image->gridimage_id}{if $image->cached_size.0 > 500}&title=on{/if}&gravity=SouthEast&hash={$image->_getAntiLeechHash()}" width=250 crossorigin="use-credentials" onerror="retryCross(this)"></a>
		</td>
	</tr>
...
{if $image->cached_size.0 > 300}
	<tr>
		<td colspan=2 align=right class="previewHover">
			Actual size:
			<div><img src="{$tile_host}/stamp.php?id={$image->gridimage_id}{if $image->cached_size.0 > 500}&title=on{/if}&gravity=SouthEast&hash={$image->_getAntiLeechHash()}" crossorigin="use-credentials" onerror="retryCross(this)">
			</div>
		</td>
	</tr>
{/if}
	...
	<tr>
		...
		<td valign=top align=center>
			<a href="/reuse.php?id={$image->gridimage_id}&amp;download={$image->_getAntiLeechHash()}"><img src="{$image->_getFullpath(true,true)}" width=250 crossorigin="use-credentials" onerror="retryCross(this)"></a><br>

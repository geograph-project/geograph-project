...
<script src="{"/slideshow.js"|revision}"></script>
	...
	<div style="position:absolute;left:-210px;top:-20px;width:220px;padding:10px;display:none;text-align:left;z-index:1000" id="showside{$image->gridimage_id}" onmouseout="hide_tree('side{$image->gridimage_id}')">
		...
		<div class="shadow shadow_large" style="clear:both; position:relative;"><a title="{$image->title|escape:'html'} - click to view image page" href="/photo/{$image->gridimage_id}">{$image->getFull(true,true,true)|replace:'src=':"name=image`$smarty.foreach.results.iteration` loading=lazy src="}</a></div>
		{if $image->comment}

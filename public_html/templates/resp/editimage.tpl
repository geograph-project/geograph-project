...
<div class="{if $image->isLandscape()}photolandscape{else}photoportrait{/if}">
	{if $image->original_width}
		<div class="caption640" style="text-align:right;  max-width:100%;"><a href="/more.php?id={$image->gridimage_id}">More sizes</a> | <a href="/resubmit.php?id={$image->gridimage_id}">Upload another large version</a></div>
  ...
  {else}
  	...
  	{/if}
  	<div class="shadow"><a href="/photo/{$image->gridimage_id}" target="_blank">{$image->getFull(true,true)}</a></div>
  {/if}
...
<div style="float:right; font-size:0.9em"><span id="styleguidelink">{newwin href="/help/style" text="Open Style Guide"}</span></div>
<br syle="clear:both;"/>
...
<div><label for="title"><b>Title</b> {if $moderated.title}<span class="moderatedlabel">(moderated)</span>{/if}</label> <br/>
...
<input type="text" id="title" name="title" style="width:100%;" value="{$image->title|escape:'html'}" title="Original: {$image->current_title|escape:'html'}" spellcheck="true" onblur="checkstyle(this,'title',true);" onkeyup="checkstyle(this,'title',false);" maxlength="128"/>
</div>
...
<textarea id="comment" name="comment" rows="7"  style="width:100%; max-width:100%;" title="Original: {$image->current_comment|escape:'html'}" spellcheck="true" onblur="checkstyle(this,'comment',true);" onkeyup="checkstyle(this,'comment',false);">{$image->comment|escape:'html'}</textarea>
...
<div><label for="view_direction"><b>View Direction</b>  {if $moderated.view_direction}<span class="moderatedlabel">(moderated)</span>{/if}
...
<div class="clearfix" style="width:100%; margin: auto;">
<div style="float:right;  position:relative"><a title="Open in Google Earth" href="/photo/{$image->gridimage_id}.kml" class="xml-kml">KML</a></div>
<br style="clear:both;"/>


	{if $rastermap->enabled}
		<div class="rastermap" style="width:{*390px*}100%; max-width:800px;">
...
<label for="grid_reference"><b style="color:#0018F8">Subject Grid Reference</b></label>
<br/>
<div style="font-size:0.8em">(required)</div>
{if $moderated.grid_reference}<span class="moderatedlabel">(moderated{if $isowner} for gridsquare changes{/if})</span>{/if}
{if $error.grid_reference}<span class="formerror">{$error.grid_reference}</span><br/>{/if}
<input type="text" id="grid_reference" name="grid_reference" size="14" style="width: 150px; max-width: 100%" value="{$image->subject_gridref|escape:'html'}" onkeyup="updateMapMarker(this,false,false)" onpaste="{literal}that=this;setTimeout(function(){updateMapMarker(that,false);},50){/literal}" onmouseup="updateMapMarker(this,false)" oninput="updateMapMarker(this,false)"/>
{if $rastermap->service != 'Google'}<img src="{$static_host}/img/icons/circle.png" alt="Marks the Subject" width="29" height="29" align="middle"/>{else}<img src="https://www.google.com/intl/en_ALL/mapfiles/marker.png" alt="Marks the Subject" width="14" height="24" align="middle"/>{/if}
<br/>
<div style="font-size:0.6em"><a href="javascript:void(resetGridRefs());">Reset to initial values</a></div>
</div>

<div style="display: inline-block; vertical-align: middle; width:250px; max-width:40%; margin:5px;">
<label for="photographer_gridref"><b style="color:#002E73">Camera Grid Reference</b></label>
<br/>
<div style="font-size:0.8em">(optional)</div>
{if $moderated.photographer_gridref}<span class="moderatedlabel">(moderated)</span>{/if}
{if $error.photographer_gridref}<span class="formerror">{$error.photographer_gridref}</span><br/>{/if}
<input type="text" id="photographer_gridref" name="photographer_gridref" size="14" style="width: 150px; max-width: 100%" value="{$image->photographer_gridref|escape:'html'}" onkeyup="updateMapMarker(this,false)" onpaste="{literal}that=this;setTimeout(function(){updateMapMarker(that,false);},50){/literal}" onmouseup="updateMapMarker(this,false)" oninput="updateMapMarker(this,false)"/>
{if $rastermap->service != 'Google'}<img src="{$static_host}/img/icons/viewc--1.png" alt="Marks the Camera" width="29" height="29" align="middle"/>{else}<img src="{$static_host}/img/icons/camicon-new.png" alt="Marks the Camera" width="14" height="24" align="middle"/>{/if}
<br/>
<div style="font-size:0.6em"><a href="javascript:void(copyGridRef());">Copy from Subject</a></div>
</div>

<div><span id="dist_message" style="padding-left:20px"></span></div>
...
<input type="checkbox" name="use6fig" id="use6fig" {if $image->use6fig} checked="checked"{/if} value="1"/> <label for="use6fig">Only display 6 figure grid reference ({newwin href="/help/map_precision" text="Explanation"})</label>
</div>
...
{*------------------------Change comments box----------------------------*}
<div class="twocolumn">
<textarea id="updatenote" name="updatenote" rows="8" style="width:100%; max-width:100%;" {if $user->message_sig} onfocus="if (this.value=='') {literal}{{/literal}this.value='{$user->message_sig|escape:'javascript'}';setCaretTo(this,0); {literal}}{/literal}"{/if}>{$updatenote|escape:'html'}</textarea>


</div>

{*------------------------Change request tickboxes----------------------------*}
<div class="twocolumn">
<br/>
<div>
...
{if $isadmin}
	...
	<input type="radio" name="mod" value="apply" id="mod_apply"/> <label for="mod_apply">Apply the changes immediately, and close the suggestion. (Contributor is notified.)</label>
  </div>

{else}
	{if $isowner}
	<div>
		<input type="checkbox" name="mod" value="pending" id="mod_pending"{if $mod_pending} checked="checked"{/if}/> <label for="mod_pending">Bring this issue to the attention of a moderator (regardless of changes made).</label>
	</div>
	{/if}
{/if}
...
<div style="display: inline-block; vertical-align: middle; width:250px; max-width:40%; margin:5px;">
<input type="submit" name="save" value="Submit Changes" onclick="autoDisable(this)" style="width: 200px; max-width: 100%; background-color: #b7e1cd;" />
</div>

<div style="display: inline-block; vertical-align: middle; width:250px; max-width:40%; margin:5px;">
<input type="button" name="cancel" value="Cancel" onclick="document.location='/photo/{$image->gridimage_id}';" style="width: 200px; max-width: 100%; background-color: #f4c7c3;"/>
</div>
...
{*------------------------End of changes form----------------------------*}
	...
	<input type="submit" value="Preview edits in a new window" onclick="previewImage()" style="width: 300px; max-width: 100%; background-color: powderblue;"/>
	</form>
...
<div id="div2" style="display:none">
	<iframe src="about:blank" height="350" width="100%" id="tagframe">
	</iframe>

...
if (!empty($_GET['q'])) {
	...
	if (!$smarty->is_cached($template, $cacheid)) {
		...
		if (count($ids)) {
			...
			$rows = $db->getAssoc("
			select tag_id,if (tag.prefix != '' and not (tag.prefix='term' or tag.prefix='category' or tag.prefix='cluster' or tag.prefix='wiki'),concat(tag.prefix,':',tag.tag),tag.tag) as tag,count(*) as images
			from tag_public tag
			where $where
			group by tag_id
			limit $limit");

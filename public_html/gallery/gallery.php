...
if ($topic_id == 7838) {
        ...
        header("Location: ".$CONF['canonical_domain'][1]."/article/Medieval-Churches-in-Norfolk");
        print "<a href=\"".$CONF['canonical_domain'][1]."/article/Medieval-Churches-in-Norfolk\">go</a>";
        exit;
} elseif (strpos($_GET['url'],'a_postcode_album') === 0) {
        header("HTTP/1.0 301 Moved Permanently");
        header("Status: 301 Moved Permanently");
        header("Location: ".$CONF['canonical_domain'][1]."/article/Postal-address-history-and-photo-album");
        print "<a href=\"".$CONF['canonical_domain'][1]."/article/Postal-address-history-and-photo-album\">go</a>";
        exit;
}

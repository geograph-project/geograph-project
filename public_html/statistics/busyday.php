...
$date = (isset($_GET['date']) && ctype_lower($_GET['date']))?intval($_GET['date']):'imagetaken';
...
{
	if ($date == 'submitted' && empty($u)) {
	        //lets hobble this! (cant use a material-view)
	...
	if (!empty($u)) {
		...
		header("Location: /statistics/groupby.php?distinct=takendays&groupby=auser_id&ri=$ri");
		exit;
	...
	if (count($where)) {
		...
		$table = $db->getAll("
		select
		DATE_FORMAT(imagetaken,'%d/%m/%Y') as Date,
		images as Images
		FROM imagetaken_stat
		ORDER BY images desc limit 50" );
	}

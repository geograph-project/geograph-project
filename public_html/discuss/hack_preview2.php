...
if (empty($CONF['disable_discuss_thumbs']) && preg_match_all("/\[\[(\[?)(\w*\d+)(\]?)\]\]/",$postText2,$g_matches)) {
	foreach ($g_matches[2] as $i => $g_id) {
		if (is_numeric($g_id)) {
			...
			$postText2 = str_replace("[[$g_id]]","<a href=\"http://{$_SERVER['HTTP_HOST']}/gridref/$g_id\" target=\"_blank\">$g_id</a>",$postText2);
		}
	}
}

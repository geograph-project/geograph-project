...
<body class="bg-gray-100 flex items-center justify-center">
    ...
    <script>
		...
		if (url.match(/auto/)) {
			...
			//actuilly now lets send to demo editor!
			url = `/stuff/mobile-demo-inner.php`;
        ...
        // NEW: Listen for messages from the iframe (the image editor)
        window.addEventListener('message', (event) => {
            if (event.data.type === 'edited_image_data') {
                const editedImageData = event.data.data;
                // Once we have the edited image data, we can move to the next step.
                // We'll set the iframe source to the submit page.
                iframe.src = '/submit-mobile.php';

                // Then, we'll wait for the new page to load and send it the data.
                iframe.onload = () => {
                    iframe.contentWindow.postMessage({ type: 'image_data', data: editedImageData }, '*');
                    // Reset the onload handler to prevent it from running again
                    iframe.onload = null;
                };
            }
        });

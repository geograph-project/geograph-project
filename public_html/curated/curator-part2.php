...
if (!empty($_POST['action'])) {
	...
	if ($_POST['action'] == 'skip') {
		$_POST['tags'] = array('Skip'); //JUST want to save a Skip !

	} else {
		if ($_POST['action'] == 'save') $_POST['tags'][] = 'Saved'; //we need to save something, so will be skipped. BUt the Saved tags should be temporary!
		if ($_POST['action'] == 'checked') $_POST['tags'][] = 'Checked';

		//FIRST DISABLE any tags that are 'offical' context
		//the query below will reenable any that ARE ticked!
		//instead COULD prhaps do AND tag NOT IN ($POST[tags]) kinda thing!
		$sql = "UPDATE curated_tag t INNER JOIN category_primary on (tag = top) SET t.status=0 WHERE t.gridimage_id = {$updates['gridimage_id']}";
		$db->Execute($sql);
...
$sql = "SELECT gi.gridimage_id, gi.user_id, title, grid_reference, gi.tags, GROUP_CONCAT(one.tag SEPARATOR '?') as done
	...
	LEFT JOIN curated_tag two ON (two.gridimage_id = gi.gridimage_id AND two.status = 1 AND two.tag IN ('Skip','Saved','Checked','Verified'))
	WHERE one.user_id = {$USER->user_id}
	AND two.gridimage_id IS NULL
...
$smarty->display('curated_curator_part2.tpl');

	...
	if ($image->isValid())
	{
		...
		if (isset($_POST['finalise']))
		{
			if($uploadmanager->setUploadId($_POST['upload_id']))
			{
				...
				$db->Execute($sql = "UPDATE gridimage_calendar SET upload_id = ".$db->Quote($_POST['upload_id']).", copied = 0
...
if (!empty($_GET['cid']))
	$smarty->assign('calendar_id',intval($_GET['cid']));

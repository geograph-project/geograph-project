...
/**
 * $Project: GeoGraph $
 * $Id: mapbrowse.php 8928 2019-04-27 14:59:42Z barry $
 * 
...
if (!$smarty->is_cached($template, $cacheid))
{
	...
	if ($gridref = $mosaic->getGridRef(-1,-1)) {
		...
		if ($mosaic->pixels_per_km == 40 && preg_match('/([A-Z]+\d)5(\d)5$/',$gridref,$m)) {
			...
			$db = GeographDatabaseConnection(true);
			$smarty->assign('hectad_row',$db->getRow("SELECT * FROM hectad_stat WHERE geosquares >= landsquares AND hectad = '$hectad' LIMIT 1"));
		}
	}

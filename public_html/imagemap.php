...
/**
 * $Project: GeoGraph $
 * $Id: imagemap.php 7525 2011-12-10 15:50:49Z barry $
 * 
...
init_session();
	...
	#USER MAP
			...
			$map->type_or_user = intval($_GET['user_id']);

	#XMAS map for a year
		} elseif ((!isset($_GET['year']) || !empty($_GET['year'])) && $year >= 2004 && $year <= date('Y')) {
			$map->setOrigin(0,-10);
	...
	#COVERAGE ANIMATION BUILDUP TILES
			...
			$start = empty($_GET['start'])?"2008-12-11":$_GET['start'];
			set_time_limit(3600*3);

			$root=&$_SERVER['DOCUMENT_ROOT'];
			$n = time()-(60*60*24*7);
			for($t=strtotime($start); $t<$n; $t+=(60*60*24*7) ) {

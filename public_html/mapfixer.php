...
/**
 * $Project: GeoGraph $
 * $Id: mapfixer.php 8782 2018-06-21 15:16:35Z barry $
 *
...
if (isset($_GET['gridref']))
{
	...
	if ($ok || ($square->x && strlen($square->grid_reference) > 4))
	{
		...
		if (count($sq))
		{
			...
			if ($count= $db->GetOne("select count(*) from mapfix_log where gridsquare_id='{$sq['gridsquare_id']}' and new_percent_land != old_percent_land"))
			{
		...
		if (isset($_GET['save']))
		{
			if ($isadmin && isset($_GET['percent_land']) && strlen($_GET['percent_land'])) {

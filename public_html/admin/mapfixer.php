...
else
{
	...
	$unknowns=$db->GetAll("select gridsquare_id,grid_reference,imagecount,
		group_concat(distinct comment order by mapfix_log_id SEPARATOR ' | ') as comments,
		group_concat(distinct concat(old_percent_land,'>',new_percent_land) order by mapfix_log_id SEPARATOR ', ') as percents
	from gridsquare

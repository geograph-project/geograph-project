...
$template = 'project_idea_edit.tpl';
$cacheid = '';
...
if ($template != 'static_404.tpl' && isset($_POST) && isset($_POST['submit'])) {
	...
	$updates = array();
	foreach (array('title','content','initiator','purpose','reason','tags','published','type','status') as $key) {
		if ($page[$key] != $_POST[$key]) {
			$updates[] = "`$key` = ".$db->Quote(trim(strip_tags($_POST[$key])));
			$smarty->assign($key, $_POST[$key]);
		} elseif (empty($_POST[$key]) && in_array($key,array('title','content','type')))
	...
	if ($_REQUEST['id'] == 'new') {
		...
		$updates[] = "`created` = NOW()";
		$sql = "INSERT INTO project_idea SET ".implode(',',$updates);
	} else {

		$sql = "UPDATE project_idea SET ".implode(',',$updates)." WHERE project_idea_id = ".$db->Quote($_REQUEST['id']);
	}

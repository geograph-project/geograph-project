...
if (!empty($_POST)) {
	...
	foreach ($_POST['complete'] as $id => $complete) {
		...
		$db->Execute($sql = "UPDATE article SET update_time=update_time,`".implode('` = ?,`',array_keys($updates)).'` = ? WHERE article_id = '.$id,array_values($updates));
	}
}
...
$data = $db->getAll("SELECT article_id,url,title,article_cat_id,licence,complete,edit_prompt,parent_url,approved FROM article WHERE approved > 0 AND user_id != {$USER->user_id} ORDER BY article_id");

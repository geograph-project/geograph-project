<VirtualHost *>
    DocumentRoot /var/www/channel_live/public_html

    #canonical domain for site
    ServerName www.geograph.org.je
    ServerAlias channel-islands.geographs.org
    ServerAlias *.channel.geographs.org
    ServerAlias *.geograph.org.gg
    ServerAlias *.geograph.org.je
    ServerAlias geograph.org.gg
    ServerAlias geograph.org.je
    ...
    CustomLog /var/www/channel_live/logs/access_log geograph env=!dontlog
    ErrorLog /var/www/channel_live/logs/error_log

    php_value include_path .:/var/www/channel_live/libs/:/usr/share/php:/usr/share/pear
    php_value register_globals Off
    php_value upload_max_filesize 8M
    php_value memory_limit  32M

    <Directory /var/www/channel_live/public_html/>
    ...
    RewriteCond %{HTTP_HOST}   !^channel-islands\.geographs\.org [NC]
    RewriteCond %{HTTP_HOST}   !^(kml|t0|s\d)\.channel\.geographs\.org [NC]
    RewriteCond %{HTTP_HOST}   !^(www|kml|t0|s\d)\.geograph\.org\.(gg|je) [NC]
    RewriteCond %{HTTP_HOST}   !^$
    RewriteRule ^/(.*)         http://channel-islands.geographs.org/$1 [L,R=permanent]

    RewriteCond %{HTTP_HOST}   ^kml\.channel\.geographs\.org [NC,OR]
    RewriteCond %{HTTP_HOST}   ^kml\.geograph\.org\.(gg|je) [NC]
    RewriteCond %{REQUEST_URI} !^/kml
    RewriteRule ^/(.*)         http://channel-islands.geographs.org/$1 [L,R=permanent]

    RewriteCond %{HTTP_HOST}   ^t0\.channel\.geographs\.org [NC,OR]
    RewriteCond %{HTTP_HOST}   ^t0\.geograph\.org\.(gg|je) [NC]
    RewriteCond %{REQUEST_URI} !^/tile
    RewriteRule ^/(.*)         http://channel-islands.geographs.org/$1 [L,R=permanent]

    RewriteCond %{HTTP_HOST}   !^channel-islands\.geographs\.org [NC]
    RewriteCond %{HTTP_HOST}   !^www\.geographs\.org\.(gg|je) [NC]
    ...
    RewriteRule /(geo|)photos/\d.*\.jpg$ http://s9.channel.geographs.org$0 [R=307,L]
...
RewriteRule ^/rss/(.*\.kmz)         http://kml.channel.geographs.org/kml/$1 [L,R=permanent]
RewriteRule ^/sitemap/geograph.*         http://channel-islands.geographs.org/list.php [L,R=permanent]

    RewriteRule ^/discuss.* /notreadyyet.php [L]
    RewriteRule ^/content.* /notreadyyet.php [L]
    RewriteRule ^/article.* /notreadyyet.php [L]
    RewriteRule ^/games.* /notreadyyet.php [L]
    RewriteRule ^/activities.* /notreadyyet.php [L]
    RewriteRule ^/teachers.* /notreadyyet.php [L]
    RewriteRule ^/chat.* /notreadyyet.php [L]
    RewriteRule ^/events.* /notreadyyet.php [L]
    RewriteRule ^/gallery.* /notreadyyet.php [L]
    RewriteRule ^/mapper/.* /notreadyyet.php [L]

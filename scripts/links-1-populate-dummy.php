	...
	foreach ($all as $url) {
		...
		if (count($rows)) {
			...
			if (!empty($found_on_image)) {
				...
				print "UPDATE gridimage_link SET ,last_found=NOW() WHERE url = $qurl\n";
				...
				print "INSERT INTO set url={$row['url']}\n";
			...
			//$db->Execute("$sql");
			print "INSERT INTO gridimage_link SET url = $qurl";
			$done++;
			print "+";
		}
		if (isset($urls[$url])) {
	...
	if (count($urls)) {
		foreach ($urls as $url => $dummy) {
			...
			$sql = "UPDATE gridimage_link SET next_check = '9999-01-01'
				...
				AND url = $qurl";
			//$db->Execute("$sql");
			print "DELETE FROM where  url = $qurl\n$sql\n";
		}
		$done++;
	}

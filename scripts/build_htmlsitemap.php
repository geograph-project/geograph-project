...
for ($sitemap=1; $sitemap<=$sitemaps; $sitemap++)
{
	...
	$filename=sprintf('%s/public_html/sitemap/sitemap%04d%s.xml', $param['dir'], $sitemap, $CONF['sitesuffix']);
	$fh=fopen($filename, "w");
...
$filename=sprintf('%s/public_html/sitemap/sitemap%s.xml', $param['dir'], $CONF['sitesuffix']);
$fh=fopen($filename, "w");

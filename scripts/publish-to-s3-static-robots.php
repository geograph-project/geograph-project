...
$param = array('verbose'=>0, 'log'=>0, 'headers'=>0, 'execute'=>0, 'overwrite'=>1);
...
$filename = 'robots-none.txt';
if ($param['config'] == 'live')
       $filename = 'robots-static.txt';
...
if ($filename) {
	...
	$source = "/".$filename; //but want one with slash sometimes
                        ...
                        if ($param['headers']==='http') {
                                ...
                                if (empty($rr))
                                        ...
                                        if (!empty($rr['class']) && $rr['class'] != 'STORAGE' && $_SERVER['HTTP_HOST'] != 'staging.geograph.org.uk') //the header typically isnt provided if STANDARD
                                                $upload = $rr['class'];
                                        if (empty($rr['size']) || $rr['size'] != filesize(".".$source))
                                                $upload = "empty";
                                }
                        }
		}

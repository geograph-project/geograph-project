...
for ($sitemap=1; $sitemap<=$sitemaps; $sitemap++) {
	...
	$recordSet = $db->Execute("select tagtext,date(last_used) as last_used from tag_stat where `count` >= 3 AND users>1 limit $offset,$urls_per_sitemap") or die($db->ErrorMsg());

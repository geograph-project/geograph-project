...
function dbExecute($sql) {
	global $db_write,$a,$param;
	if ($param['execute']) {
		$db_write->Execute($sql) or die($sql."\n".$db->ErrorMsg()."\n\n");
		$a += $db_write->Affected_Rows();
	} else {
		print "$sql;\n";
	}
}
...
dbExecute("DELETE FROM tpoints WHERE grid_reference = '$square'");
			}
...
dbExecute("INSERT IGNORE INTO tpoints SET gridimage_id = ".$recordSet->fields['gridimage_id'].",grid_reference = '$square',created = NOW()");
...
-- just to create the table with the right columns (all filled in!)
create table tpoints_check select gridimage_id, grid_reference, created, points from gridimage left join tpoints  using (gridimage_id)
 where points = 'tpoint' and tpoints.gridimage_id is not null limit 10;

-- clear out
truncate tpoints_check;

-- insert where labeled as tpoint incorrectly!
insert into tpoints_check select gridimage_id, gridimage_search.grid_reference, created, points from gridimage_search left join tpoints  using (gridimage_id)
 where points = 'tpoint' and tpoints.gridimage_id is null ;

-- and where not labled as tpoint - also incorrectly!
insert into tpoints_check select gridimage_id, tpoints.grid_reference, created, points from tpoints left join gridimage_search using (gridimage_id)
 where points != 'tpoint';

--alternate (although SHOULD be tehe same!)
 select gridimage_id, '?' AS grid_reference, created, points from tpoints left join gridimage using (gridimage_id)  where points != 'tpoint';


-- shouldnt be any of these!
insert into tpoints_check select gridimage_id, tpoints.grid_reference, created, points from tpoints left join gridimage_search using (gridimage_id)

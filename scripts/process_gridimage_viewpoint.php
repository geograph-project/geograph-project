...
for($start = 1; $start <= $max; $start += $limit) {
	...
	$recordSet = $db->Execute("SELECT gridimage_id,viewpoint_eastings,viewpoint_northings,gs.reference_index,viewpoint_refindex
	...
	while (!$recordSet->EOF) {
		...
		list($lat,$long) = $conv->national_to_wgs84($recordSet->fields['viewpoint_eastings'],$recordSet->fields['viewpoint_northings'],
			$recordSet->fields['viewpoint_refindex']);

...
def parseosm(file):
	...
	# </way>								7->8				7	8	4
	# <relation ...>							8->9				8	9
	#   <member type="way" ref="4096484" role="outer"/>			9,10->10			9,10	10	5
	#   <member type="way" ref="4096487" role=""/>
	#   <tag k="created_by" v="almien_coastlines"/>
	# </relation ...>							10->11				10	11
	#</osm>									11->12				11	12
	#EOF									12->X				12	X	X
	...
	for line in file:
		...
		if action == 0:
			...
			waynodes = []
			if wayid in ways:
				print >> sys.stderr, "duplicate way", id
				sys.exit(1)
		elif action == 3:
			id = int(m.group(1))
			coord = nodes[id]
...
def convertpoly(filename, outfile, zone, x1, y1, x2, y2, cx1, cy1, cx2, cy2, ds, eps, graylow, grayhigh, epscomb):
	...
	#for poly in polygons:
	#	poly[1] = 1      # only outer lines here, data is inaccurate # FIXME seems to work now
...
def mainprog(argv = None):
	if argv is None:
		argv = sys.argv
	if len(argv) < 8 or len(argv) in [9, 10, 11] or len(argv) > 17:
		print >>sys.stderr, 'USAGE: poly.py osm_xml_file png_file utm_zone xmin ymin xmax ymax [ clipxmin clipymin clipxmax clipymax [ ds [ eps [ graylow [ grayhigh [ epscomb ]]]]]]'
		return 1
	filename = argv[1]
	outfile = argv[2]
	...
	convertpoly(filename, outfile, zone, x1, y1, x2, y2, cx1, cy1, cx2, cy2, ds, eps, graylow, grayhigh, epscomb)
	return 0

...
class RebuildRegionStat extends EventHandler
{
	function processEvent(&$event)
	{
		...
		$db->Execute("
insert into loc_hier_stat_tmp (level,community_id,squares_total,squares_geo,images_total,squares_submitted,tenk_total,geographs_submitted,tenk_submitted,images_thisweek) select * from (
	select level,community_id,count(*) as squares_total,sum(permit_geographs>0) as squares_geo,sum(imagecount) as images_total,sum(imagecount > 0) as squares_submitted, count(distinct concat(substring(grid_reference,1,length(grid_reference)-3),substring(grid_reference,-2,1))) as tenk_total from gridsquare gs inner join gridsquare_percentage gp on (gs.gridsquare_id=gp.gridsquare_id) where level >= 0 and percent > 0 and percent_land > 0 group by level,community_id
) as da left join (

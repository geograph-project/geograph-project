...
<p>This is a measure of the number of times the page has been viewed. <b>It is almost certainly an under estimate</b>, as it doesn't truly represent how many times the photo itself has been viewed. It only counts the number of times the main page for the image has been viewed. The count is historic and records the number of times that the page has been viewed over the last four or so years (unfortunately some data was lost from early in the project).</p>

<p>A typical photo is visible in many more places, such as Wikipedia and other websites that re-use our photos, often in bulk. Also, Geograph's own site features such as the search engine {if $enable_forums} and forum{/if} are not taken into account.</p>

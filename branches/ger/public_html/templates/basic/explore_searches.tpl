...
<tr{if !$row.approved} style="color:gray;text-decoration:line-through"{/if}>
	<td><a href="/search.php?i={$row.id}">images{$row.searchdesc|escape:'html'}</a> {if $row.count}[{$row.count|thousends}]{/if}{if $row.comment}<br/><small><small>{$row.comment|escape:'html'}</small></small>{/if}</td>

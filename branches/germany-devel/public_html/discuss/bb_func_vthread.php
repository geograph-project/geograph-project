...
FUNC;
	...
	if ($USER->user_id) {
		...
		$myRes=$db->Execute("insert into geobb_lastviewed set topic_id=$topic,user_id={$USER->user_id},last_post_id = $postID on duplicate key update last_post_id = if(last_post_id < $postID,$postID,last_post_id)") or die('<p>'.$db->ErrorMsg().'. Please, try another name or value.');
	}

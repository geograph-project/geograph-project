...
if (empty($_REQUEST['location_id']) && (empty($_REQUEST['page']) || preg_match('/[^\w\.\,-]/',$_REQUEST['page']))) {
	$smarty->display('static_404.tpl');
	exit;
}


$template = 'location_edit2.tpl';
$cacheid = '';



	$db=NewADOConnection($GLOBALS['DSN']);
	if ($_REQUEST['page'] == 'new' || $_REQUEST['location_id'] == 'new') {
		$smarty->assign('location_id', "new");
		$smarty->assign('title', "New Location");
		$smarty->assign('realname', $USER->realname);
		$smarty->assign('user_id', $USER->user_id);
		$page = array();
	} else {
		if (!empty($_REQUEST['location_id'])) {
			$sql_where = " location_id = ".$db->Quote($_REQUEST['location_id']);
		} else {
		...
		$page = $db->getRow("
		select location.*,realname,gs.grid_reference
		from location 
			left join user using (user_id)
			left join gridsquare gs on (location.gridsquare_id = gs.gridsquare_id)
		where $sql_where
		limit 1");
		...
		if (count($page) && (
			...
			if (isset($_GET['release'])) {
				$db->Execute("DELETE FROM location_lock WHERE user_id = {$USER->user_id} AND location_id = {$page['location_id']}");
			...
			$smarty->assign($page);
			$db->Execute("REPLACE INTO location_lock SET user_id = {$USER->user_id}, location_id = {$page['location_id']}");
		} else {
...
if ($template != 'static_404.tpl' && isset($_POST) && isset($_POST['submit'])) {
	...
	if ($page['approved'] == 2) {
		...
		$keys = array('url','title','licence','content','publish_date','location_cat_id','gridsquare_id','extract');
	}
	...
	foreach ($keys as $key) {
		if ($page[$key] != $_POST[$key]) {
			...
			if ($key == 'url' || $key = 'title') {
				$sql = "select count(*) from location where `$key` = ".$db->Quote($_POST[$key]);
				if (!empty($_REQUEST['location_id'])) {
	...
	if ($_REQUEST['page'] == 'new' || $_REQUEST['location_id'] == 'new') {
		...
		$sql = "INSERT INTO location SET ".implode(',',$updates);
	} else {
		
		$sql = "UPDATE location SET ".implode(',',$updates)." WHERE location_id = ".$db->Quote($_REQUEST['location_id']);
	}
	if (!count($errors) && count($updates)) {
		
		$db->Execute($sql);
		if ($_REQUEST['page'] == 'new' || $_REQUEST['location_id'] == 'new') {
			$_REQUEST['location_id'] = $db->Insert_ID();
		}

		require_once('geograph/event.class.php');
		new Event("location_updated", $_REQUEST['location_id']);

		//and back it up
		$sql = "INSERT INTO location_revisions SELECT *,NULL,{$USER->user_id} FROM location WHERE location_id = ".$db->Quote($_REQUEST['location_id']);
		$db->Execute($sql);

		$_SESSION[$_POST['url']] = $db->Insert_ID();

		$smarty->clear_cache('location_location.tpl', $_POST['url']);
		$smarty->clear_cache('location.tpl');

		$db->Execute("DELETE FROM location_lock WHERE user_id = {$USER->user_id} AND location_id = {$_REQUEST['location_id']}");

		header("Location: /location/".(empty($_POST['url'])?$page['url']:$_POST['url']));
		exit;
	} else {
		if ($errors[1] != 1)

...
<br/><a href="/search.php?taken_endMonth={$month}&amp;taken_endYear={$year}&amp;taken_startMonth={$month}&amp;taken_startYear={$year}&amp;orderby=imagetaken&amp;do=1">Alle Bilder vom {$month_name} {$year} suchen</a>		
...
<tr>
	{foreach from=$week item=day}
		...
		{if $day.number}
			...
			{if $day.image}
				(<a href="/search.php?taken_endDay={$day.number}&amp;taken_endMonth={$month}&amp;taken_endYear={$year}&amp;taken_startDay={$day.number}&amp;taken_startMonth={$month}&amp;taken_startYear={$year}&amp;orderby=imagetaken&amp;do=1">mehr</a>)</div>
				
				<div style="text-align:center;width:120px;height:120px;vertical-align:middle">
					<a title="{$day.image->grid_reference} : {$day.image->title|escape:'html'} von {$day.image->realname|escape:'html'} - zum Vergrößern anklicken" href="/photo/{$day.image->gridimage_id}">{$day.image->getThumbnail(120,120)}</a>
				</div>

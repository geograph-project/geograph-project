...
{if $engine->resultCount}
	...
	{foreach from=$engine->results item=image name=results}
	 ...
	 <div style="float:right; position:relative;">{$smarty.foreach.results.iteration}/{$engine->numberofimages}</div>
		...
		<div class="img-shadow" style="clear:both; position:relative;"><a title="{$image->title|escape:'html'} - click to view image page" href="/photo/{$image->gridimage_id}">{$image->getFull()|replace:'src=':"name=\"image`$smarty.foreach.results.iteration`\" data-lowsrc="}</a></div>
		{if $image->comment}
...
setTimeout("document.images['image1'].src = document.images['image1'].getAttribute('data-lowsrc')",300);
setTimeout("document.images['image2'].src = document.images['image2'].getAttribute('data-lowsrc')",600);
{dynamic}

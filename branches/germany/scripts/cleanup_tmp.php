...
#while (1) {
	...
	if ($dh !== false) {
		...
		{
			if (!fnmatch('newpic_u*', $filename))
				continue;
			$fullname = $tmpdir.'/'.$filename;
			#FIXME use stat
			if (!is_file($fullname))
				continue;
			if (filemtime($fullname) > $deltime)
				continue;
			if (filectime($fullname) > $deltime)
				continue;

			print "deleting: $fullname ". date ("F d Y H:i:s", filemtime($fullname))."/".date ("F d Y H:i:s", filectime($fullname))."\n";
			unlink($fullname);

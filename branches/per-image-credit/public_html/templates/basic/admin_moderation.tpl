...
{if $unmoderatedcount}
	...
	{/if} 
	{if !$moderator}
	Simply look at each image in turn and click the relevant button. The result of the action is displayed just below the button. <big><b>Wait for result before moving onto the next button.</b></big> <br/><br/>
	...
	{foreach from=$unmoderated item=image}
	  ...
	  by: {if $image->credit_realname}<b style="color:blue">{$image->credit_realname}</b>, submitted by {/if}<b><a title="view user profile" href="/profile.php?u={$image->user_id}">{$image->realname}</a></b> (rank: {$image->rank})<br/>
	  ...
	  {/if}
	  <div class="caption" id="modinfo{$image->gridimage_id}">{if $image->user_status}User: suggest {$image->user_status}{else}{if $image->different_square}System: suggest Accept{else}&nbsp;{/if}{/if}</div>
	  </div>
	  <br style="clear:both"/>
	  </div>

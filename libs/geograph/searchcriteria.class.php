...
/**
 * $Project: GeoGraph $
 * $Id: searchcriteria.class.php 8937 2019-05-10 23:15:23Z barry $
 * 
...
class SearchCriteria
{
	...
	*/
	...
	function &_getDB($allow_readonly = false)
	{
		...
		if (!is_object($this->db) || (!empty($this->db->readonly) && !$allow_readonly) ) {
...
#split_timer('search'); //starts the timer
		...
		if (count($places) == 1) {
			...
			$this->placename = $places[0]['full_name'].((!empty($places[0]['adm1_name']))?", {$places[0]['adm1_name']}":'');

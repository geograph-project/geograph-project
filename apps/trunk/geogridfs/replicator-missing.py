...
#
# checks all files that SHOULD be on the target, and replicaes them if not (will also replace any existing!) 
#
##
#    Copyright (C) 2018  Barry Hunter  <geo@barryhunter.co.uk>
#
...
def replicate_now(path = '', target = '', order = ''):
  ...
  cc=db.cursor(MySQLdb.cursors.DictCursor)
  c=db.cursor(MySQLdb.cursors.DictCursor)
  cex=db.cursor()

  cc.execute("SELECT * FROM file_stat WHERE replicas LIKE '%"+target+"%' AND replica_count > 1 GROUP BY shard ORDER BY "+order)
  ...
  while True:
    ...
    crit = "file_id BETWEEN "+str(start)+" AND "+str(end)+" AND replica_count > 1"

    print "SELECT file_id,filename,replicas,size,md5sum FROM "+config.database['file_table']+" WHERE replicas & "+str(idx)+" AND "+crit
    
    c.execute("SELECT file_id,filename,replicas,size,md5sum FROM "+config.database['file_table']+" WHERE replicas & "+str(idx)+" AND "+crit)
    ...
    while True:
        ...
        for key in replicas:
            if key != target and os.path.exists(config.mounts[key] + row['filename']):
                replica = key
                break
        ...
        #    print "BUT dates doesnt match '"+str(stat.st_mtime)+"' != '"+str(row['modified'])+"'"
            ...
            #include the replica filter, as we should have a file on this replica make it a NOOP
...
def main(argv):
    ...
    try:
        ...
        print 'replicator-missing.py -r cream'
        sys.exit(2)
    ...
    if action == 'unknown' or not replica:
        print 'replicator-missing.py -r cream'
        sys.exit(2)

		...
		if (is_numeric($g_id)) {
			...
			if ($ok) {
				if ($g_matches[1][$i]) {
					...
					$title=$g_image->grid_reference.' : '.htmlentities($g_image->title);
					$posterText = str_replace("[[[$g_id]]]","<a href=\"http://{$_SERVER['HTTP_HOST']}/photo/$g_id\" target=\"_blank\" title=\"$title\">$g_img</a>",$posterText);
				} else {

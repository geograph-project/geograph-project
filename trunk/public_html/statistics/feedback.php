...
if (!$smarty->is_cached($template, $cacheid)) {
	...
	$table = array();
		...
		$table['table']=$db->GetAll("
		select category,question,sum(vote=-2) as 'Didn\'t Know',sum(vote=-1) as 'Not Tried',sum(vote>0) as Votes,sum(if(vote>0,vote,0))/sum(vote>0) as Average from feedback inner join vote_log using (id) where type = 'f' and category != 'Experience' group by id order by 4
		");

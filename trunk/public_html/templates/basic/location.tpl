...
<ul style="list-style-type:none; padding-left:5px">
	<li><img src="http://{$static_host}/img/links/submit.png" width="20" height="20" alt="submit icon" align="absmiddle"/> <a href="/submit.php?gridreference={$gridrefraw}"><b>submit your own picture for {$gridref}</b></a></li>
	<li><img src="http://{$static_host}/img/links/browse.png" width="20" height="20" alt="browse icon" align="absmiddle"/> <a href="/gridref/{$gridrefraw}">View <b>browse page</b> for {$gridref}</a> 
		{if $square->imagecount > 2}
				(<img src="http://{$static_host}/img/links/grid.png" width="20" height="20" alt="centisquare icon" align="absmiddle"/>  <a href="/gridref/{$gridref}?by=1">view <b>breakdown</b></a>)
			{/if}</li>
	{if $square->imagecount > 2}
		<li><img src="http://{$static_host}/img/links/slideshow.png" width="20" height="20" alt="slideshow icon" align="absmiddle"/> <a href="/search.php?gridref={$gridref}&amp;distance=1&amp;displayclass=slide&amp;orderby=submitted&amp;do=1" title="View images in a Slide Show" class="nowrap">View <b>slide show</b> for the {$square->imagecount} images in this square</a></li>
	{/if}
	{if $viewpoint_count}
		<li><img src="http://{$static_host}/img/links/lookout.png" width="20" height="20" alt="browse icon" align="absmiddle"/> <a href="/full-text.php?q={$viewpoint_query|escape:'url'}">view <b>{$viewpoint_count} image{if $viewpoint_count != 1}s{/if} taken <i>from</i> {$gridref}</b></a></li>
	{/if}
	{if $enable_forums}
		<li><img src="http://{$static_host}/img/links/discuss.png" width="20" height="20" alt="discussion icon" align="absmiddle"/> 
		{if $discuss}
		...
		{/if}</li>
	{/if}
	<li><img src="http://{$static_host}/img/links/gpx.png" width="20" height="20" alt="gpx icon" align="absmiddle"/> <a title="Download GPX" href="/gpx.php?gridref={$gridref}">Download a <b>GPX coverage</b> file around {$gridref}</a></li>
	{if strlen($gridrefraw) < 5}
		<li><img src="http://{$static_host}/img/links/hectad.png" width="20" height="20" alt="hectad icon" align="absmiddle"/> <a title="First Geographs within {$gridrefraw}" href="/search.php?first={$gridrefraw}">Find <b>first geographs for hectad</b> {$gridrefraw}</a></li>
	{/if}
	<li><img src="http://{$static_host}/img/links/search.png" width="20" height="20" alt="search icon" align="absmiddle"/> <a title="search for nearby images to {$gridref}" href="/search.php?q={$gridref}"><b>search</b> for images near {$gridref}</a></li>
	<li><img src="http://{$static_host}/img/links/no-photos.png" width="20" height="20" alt="no photos icon" align="absmiddle"/> <a title="Empty Squares" href="/squares.php?gridref={$gridref}&amp;type=without">View list of nearby <b>squares without images</b></a><br/><br/></li>
	
	{if $gridref6}
		<li><img src="http://{$static_host}/img/links/centi.png" width="20" height="20" alt="centisquare icon" align="absmiddle"/> <a href="/gridref/{$gridref}?viewpoint={$gridref6}">image(s) <b>taken in {$gridref6}</b></b></a> / <span class="nowrap"><a href="/gridref/{$gridref}?centi={$gridref6}">of <b>subjects in {$gridref6}</b></a> (if any)</span><br/><br/></li>
	{/if}
	
	<li style="list-style-type:none"><img src="http://{$static_host}/img/links/map.png" width="20" height="20" alt="browse icon" align="absmiddle"/> <a href="/mapbrowse.php?t={$map_token}&amp;gridref_from={$gridref}">Geograph <b>map</b></a>, <img src="http://{$static_host}/img/links/depth.png" width="20" height="20" alt="browse icon" align="absmiddle"/> <a href="/mapbrowse.php?t={$map_token}&amp;gridref_from={$gridref}&amp;depth=1"><b>Depth</b> Version</a>{if $square->reference_index == 1}, <img src="http://{$static_host}/img/links/mapper.png" width="20" height="20" alt="browse icon" align="absmiddle"/> <a href="/mapper/?t={$map_token}&amp;gridref_from={$gridref}"><b>Draggable</b></a><sup style="color:red">New!</sup>{/if} </li>
	<li style="list-style-type:none"><img src="http://{$static_host}/img/links/checksheet.png" width="20" height="20" alt="browse icon" align="absmiddle"/> <a title="show a print friendly page you can use&#13;&#10;to check off the squares you photograph&#13;&#10;while in the field" href="/mapsheet.php?t={$map_token}&amp;gridref_from={$gridref}">View a <b>printable check sheet</b> for {if strlen($gridrefraw) < 5}{$gridrefraw}{else}{$gridref}{/if}</a></li>
...
<ul style="margin-top:0px; margin-bottom:0px">
	...
	{if $square->reference_index eq 1}
		<li>{external href="http://www.magic.gov.uk/website/magic/opener.htm?startTopic=maggb&xygridref=`$square->nateastings`,`$square->natnorthings`&startscale=10000" text="magic.gov.uk"}</li> 
...
<ul style="margin-top:0px">
	...
	{if $square->reference_index eq 1}
			...
			<li>{external title="find local features and maps with wikimedia" href="http://tools.wikimedia.de/~magnus/geo/geohack.php?params=`$lat`_`$nl`_`$long`_`$el`_region:GB_scale:25000" text="<b>more</b> from wikimedia"}<br/><br/></li>
